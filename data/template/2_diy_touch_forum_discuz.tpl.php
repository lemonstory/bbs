...
<?php } ?>
<!-- modify by gaoyong 2015.12.18 去掉手机版头部
<header class="header" style="overflow:hidden;margin-bottom:10px">
<div class="hdc cl">
<?php if($_G['setting']['domain']['app']['mobile']) { $nav = 'http://'.$_G['setting']['domain']['app']['mobile'];?><?php } else { $nav = "forum.php";?><?php } ?>
<h2><a title="<?php echo $_G['setting']['bbname'];?>" href="<?php echo $nav;?>" class="logo"><img src="<?php echo STATICURL;?>image/mobile/images/logo.png" /></a></h2>
<ul class="user_fun">
...
<!-- modify by gaoyong 2015.12.18 去掉分区信息，目前只用一个分区
<div class="subforumshow bm_h cl" href="#sub_forum_<?php echo $cat['fid'];?>">
<span class="o"><img src="template/jiuyue/touch/images/collapsed_<?php if(!$_G['setting']['mobile']['mobileforumview']) { ?>yes<?php } else { ?>no<?php } ?>.png"></span>
<h2><a href="javascript:;"><?php echo $cat['name'];?></a></h2>
</div>
-->
<div id="sub_forum_<?php echo $cat['fid'];?>" class="sub_forum">
<ul class="cl"><?php if(is_array($cat['forums'])) foreach($cat['forums'] as $forumid) { $forum=$forumlist[$forumid];?><li class="cl">
                    <?php if($forum['icon']) { ?>
                        <?php echo $forum['icon'];?>
                    <?php } else { ?>
                        <a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $forum['fid'];?>"><img src="<?php echo IMGDIR;?>/forum<?php if($forum['folder']) { ?>_new<?php } ?>.gif" alt="<?php echo $forum['name'];?>" /></a>
                    <?php } ?>
                    <a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $forum['fid'];?>" class="f-name"><?php echo $forum['name'];?></a>
                </li>
<?php } ?>
</ul>
</div>
...
obj.find('img').attr('src', 'template/jiuyue/touch/images/collapsed_yes.png');
} else {
subobj.css('display', 'none');
obj.find('img').attr('src', 'template/jiuyue/touch/images/collapsed_no.png');
}
});
 })();
</script>
