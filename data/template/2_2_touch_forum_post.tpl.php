...
0
|| checktplrefresh('./template/jiuyue/touch/forum/post.htm', './template/jiuyue/touch/common/seccheck.htm', 1450450202, '2', './data/template/2_2_touch_forum_post.tpl.php', './template/jiuyue', 'touch/forum/post')
...
<!-- header start -->
<header class="header">
<span class="y"><button id="postsubmit" class="btn_pn <?php if($_GET['action'] == 'edit') { ?>btn_pn_blue<?php } else { ?>btn_pn_grey<?php } ?>" disable="<?php if($_GET['action'] == 'edit') { ?>false<?php } else { } ?>true"><span><?php if($_GET['action'] == 'newthread') { ?>发表<?php } elseif($_GET['action'] == 'reply') { ?>回复<?php } elseif($_GET['action'] == 'edit') { ?>保存<?php } ?></span></button></span>
...
<a href="<?php if($_GET['action'] == 'newthread') { ?>forum.php?mod=forumdisplay&fid=<?php echo $_G['fid'];?>&page=<?php echo $_GET['page'];?><?php } else { ?>forum.php?mod=redirect&goto=findpost&ptid=<?php echo $_G['tid'];?>&pid=<?php echo $pid;?><?php } ?>" class="back icon"></a>
<span class="name"><?php if($_GET['action'] == 'edit') { ?>编辑<?php } else { ?>发帖<?php } ?></span>
</header>
<!-- header end -->
...
<textarea class="pt" id="needmessage" tabindex="3" autocomplete="off" id="<?php echo $editorid;?>_textarea" name="<?php echo $editor['textarea'];?>" cols="80" rows="5"  placeholder="内容" fwin="reply"><?php echo $postinfo['message'];?></textarea>
</li>
